...
static void tfs_lookup(old_fuse_req_t req, fuse_ino_t parent,
                       const char *name) {
    ...
    if (tfs_stat(e.ino, &e.attr) != 0)
        goto err_out;
    old_fuse_reply_entry(req, &e);
    return;

err_out:
    old_fuse_reply_err(req, ENOENT);
}

static void tfs_getattr(old_fuse_req_t req, fuse_ino_t ino,
    ...
    if (tfs_stat(ino, &stbuf) != 0)
        ...
        old_fuse_reply_attr(req, &stbuf, 5);
}

static void tfs_open(old_fuse_req_t req, fuse_ino_t ino,
                     struct fuse_file_info *fi) {
    if (ino == FUSE_ROOT_ID)
        old_fuse_reply_err(req, EISDIR);
    else {
        assert(ino == FILE_INO);
        fi->fh = FILE_INO;
...
static void tfs_setattr (old_fuse_req_t req, fuse_ino_t ino, struct stat *attr,
                         int to_set, struct fuse_file_info *fi) {
    if(ino != FILE_INO ||
       !(to_set & FUSE_SET_ATTR_MODE)) {
        old_fuse_reply_err(req, EINVAL);
        return;
    }
...
static struct fuse_lowlevel_ops tfs_oper = {
    .old_lookup	= tfs_lookup,
    .old_getattr	= tfs_getattr,
    .old_open	= tfs_open,
    .old_setattr	= tfs_setattr,
};

static void* run_fs(void *data) {
    struct fuse_session *se = (struct fuse_session*) data;
    assert(old_fuse_session_loop(se) == 0);
    return NULL;
}
