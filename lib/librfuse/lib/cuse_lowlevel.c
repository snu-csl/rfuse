...
static struct cuse_lowlevel_ops *req_clop(old_fuse_req_t req)
{
	return &req->se->cuse_data->clop;
}

static void cuse_fll_open(old_fuse_req_t req, fuse_ino_t ino,
...
static void cuse_fll_read(old_fuse_req_t req, fuse_ino_t ino, size_t size,
...
static void cuse_fll_write(old_fuse_req_t req, fuse_ino_t ino, const char *buf,
...
static void cuse_fll_flush(old_fuse_req_t req, fuse_ino_t ino,
			   struct fuse_file_info *fi)
{
	(void)ino;
...
static void cuse_fll_release(old_fuse_req_t req, fuse_ino_t ino,
			     struct fuse_file_info *fi)
{
	(void)ino;
...
static void cuse_fll_fsync(old_fuse_req_t req, fuse_ino_t ino, int datasync,
			   struct fuse_file_info *fi)
{
	(void)ino;
...
static void cuse_fll_ioctl(old_fuse_req_t req, fuse_ino_t ino, unsigned int cmd, void *arg,
...
static void cuse_fll_poll(old_fuse_req_t req, fuse_ino_t ino,
...
static int cuse_reply_init(old_fuse_req_t req, struct cuse_init_out *arg,
...
void cuse_lowlevel_init(old_fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
		       ...
		       const struct cuse_lowlevel_ops *clop, void *userdata)
{
	...
	if (multithreaded) {
		...
		res = old_fuse_session_loop_mt_32(se, &config);
	}
	else
		res = old_fuse_session_loop(se);
